...
namespace Coldstore
{
    public partial class SelectAcc : Form
    {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dataGridView1.Rows.Count != 0 && dataGridView1.CurrentCell.RowIndex == 0)
                {
                    ...
                    if (row == "row")
                    {
                        ...
                        for (int i = 0; i < 7; i++)
                        {
                            outStr = outStr + dataGridView1.SelectedRows[0].Cells[i].Value.ToString() + "|";
                        }
        ...
        private void SelectAcc_FormClosing(object sender, FormClosingEventArgs e)
        {
            row = "";
        }

       // internal void Select(DataTable dt, string selectedText, int uptoIndex)
        //{
